...
def get_missing_keywords(resume_text, job_description):
    ...
    common_fillers = {'with', 'from', 'that', 'this', 'your', 'have', 'experience', 'knowledge', 'the', 'and', 'for', 'skills', 'required'}
    suggested_skills = [word for word in missing if len(word) > 3 and word not in common_fillers]
    
    return suggested_skills[:10]


def get_job_platform_links(job_description):
    """Return application search URLs for major job platforms based on JD keywords."""
    if not job_description or not job_description.strip():
        return {}

    tokens = re.findall(r"\w+", job_description.lower())
    stop_words = {
        'a', 'an', 'the', 'and', 'or', 'for', 'to', 'with', 'in', 'on', 'at', 'by', 'of',
        'role', 'job', 'position', 'experience', 'skills', 'requirements', 'requirements',
        'you', 'your', 'work', 'requirements', 'candidate', 'should', 'must'
    }
